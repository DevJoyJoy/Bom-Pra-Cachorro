...
export const RegisterPet = () => {
  ...
  const handleRegisterPet = async () => {
    try {
      ...
      if (!name || !animal || !breed) {
        alert("Preencha os campos obrigatórios.");
        return;
      }

      const uploadImage = async (imageObj, index) => {
        if (!imageObj.file) return null;

        const imageRef = ref(storage, `pets/${uid}/${Date.now()}_${index}`);

        await uploadBytes(imageRef, imageObj.file);
  ...
  return (
    ...
    >
      ...
      >
        <h1
          className="pt-3 lg:pb-3 font-bold breeSerif
            text-2xl 
            md:text-4xl 
            lg:text-3xl"
        >
        ...
        >
          ...
          >
            ...
            <label
              className="relative flex flex-col items-center justify-center overflow-hidden border-4 border-dashed border-[#4b4b4b] rounded-2xl cursor-pointer transition-colors hover:bg-[#e7e7e7]
                w-[90%] h-[75%] 
                lg:w-full lg:h-[70%]"
            >
              {image1.preview ? (
                ...
                <>
                  ...
                  <span className="mt-2 text-[#4b4b4b] md:text-3xl lg:text-2xl">
                    Foto do Pet
                  </span>
                </>
              )}
            ...
            <div
              className="flex flex-row justify-center mt-3 gap-12
                w-full 
                lg:gap-7"
            >
              {/* Second image input */}
              ...
              >
                {image2.preview ? (
                  ...
                  <span className="text-xl text-[#4b4b4b] md:text-5xl lg:text-3xl">
                    +
                  </span>
                )}
                <input
          ...
          >
            <div className="w-[45%] p-3">
              ...
              <input
                ...
                className="rounded-xl w-[110%] lg:w-[90%] md:text-3xl lg:text-xl bg-[#cfcccc]"
              />
              <br />
              <br />
              <div className="flex flex-row justify-between lg:w-[90%]">
                ...
                <button
                  onClick={() => setIsAnimalsOpen(true)}
                  className="text-[100%] md:text-4xl lg:text-xl"
                >
                  +
                </button>
              </div>
              <select
                type="text"
                className="rounded-xl w-[110%] md:w-[110%] lg:w-[90%] md:text-3xl lg:text-xl bg-[#cfcccc]"
              />
              <br />
              <br />
              <div className="flex flex-row justify-between lg:w-[90%]">
            ...
            <div className="w-[55%] p-3">
              <p className="text-[100%] md:text-3xl lg:text-xl">
                Data de nascimento
              </p>
              <input
                type="date"
                onChange={(e) => setBirthDate(e.target.value)}
                className="rounded-xl w-full lg:w-[90%] md:text-3xl lg:text-xl bg-[#cfcccc]"
              />
              <br />
              <br />
              <div className="flex flex-row">
                ...
                <input
                  ...
                  className="w-[30%] md:text-3xl bg-[#cfcccc]"
                />
              </div>
              <br />
              <div className="flex flex-row">
                <p className="text-[100%] mr-3 md:text-3xl lg:mr-9.5 lg:text-xl">
                  Sexo
                </p>
                ...
                <input
                  ...
                  className="rounded-xl w-[30%] bg-[#cfcccc]"
                />
              ...
              <br />
              <p className="text-[100%] mr-3 md:text-3xl lg:text-xl">
                Descrição
              </p>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                className="rounded-xl w-[105%] lg:w-[90%] h-[33%] lg:h-[37%] md:text-3xl lg:text-xl bg-[#cfcccc]"
              />
            </div>
          </div>
        </div>
        <div className="flex justify-center w-full h-[8%] md:h-22 lg:h-[10%] lg:mt-7 lg:w-[50%]">
          <button
            ...
            className="px-6 py-2 md:w-[70%] lg:w-[70%] md:h-full text-white font-bold md:text-4xl lg:text-xl rounded-full cursor-pointer transition-colors bg-[#0097b2] hover:bg-[#015b6b] arturo"
          >
            Salvar e cadastrar
          </button>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-1 flex items-center justify-center">
          ...
          {/* Modal white box */}
          <div
            className="z-1 flex flex-col items-center p-8 bg-white rounded-2xl
            w-[80%] 
            lg:w-[40%]"
          >
            ...
            <div className="flex flex-col items-center justify-between w-full">
              <button
                ...
                className="px-6 py-2 text-white font-bold rounded-full cursor-pointer transition-colors bg-[#0097b2] hover:bg-[#015b6b] arturo
                md:text-2xl 
                lg:text-xl  lg:w-[70%]"
              >
                Voltar aos pets
      ...
      {isAnimalsOpen && (
        <div className="fixed inset-0 z-1 flex items-center justify-center">
          {/* Dark background */}
          <div
            onClick={() => setIsAnimalsOpen(false)}
            className="absolute inset-0 bg-black opacity-60"
          ></div>

          {/* Modal white box */}
          <div className="z-1 flex flex-col items-center w-[80%] lg:w-[40%] p-8 bg-white rounded-2xl">
            <p className="mb-6 text-center md:text-2xl">
              Digite o novo animal:
            </p>
            <input
              type="text"
              className="w-[110%] md:w-[90%] lg:w-[90%] p-[1vh] bg-[#cfcccc] rounded-2xl"
            />
            <br />
            <button
              onClick={() => setIsAnimalsOpen(false)}
              className="px-6 py-2 text-white font-bold md:text-2xl lg:text-xl rounded-full cursor-pointer transition-colors bg-[#0097b2] hover:bg-[#015b6b] arturo"
            >
              Cadastrar animal
            </button>
          </div>
